...
def msgFilter(SecDescription,line):    
    if SecDescription == "":
        raise ValueError("SecDescription: Missing security description variable.")
    secDesc = b'\x01107='+SecDescription.encode()+b'\x01'
    tradeType = line[line.find(b'\x01269=')+5:line.find(b'\x01269=')+6] in b'0|1'
    mkRefresh = line[line.find(b'\x0135=')+4:line.find(b'\x0135=')+5] in b'\x0135=X\x01'
    if mkRefresh and tradeType and secDesc in line:
        return line    
    ...
    def buildbook(self,SecDescription,fileOut=False):
        ...
        if fileOut==False:
            ...
            ############################ BOOK UPDATE  ###########################
                ...
                self.books.append(book_header + book_end)
        self.data.seek(0)
        return self.books
